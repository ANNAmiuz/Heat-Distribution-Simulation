...
int main(int argc, char **argv)
{
    ...
    workload_distributor(displs, scounts, current_state.room_size, THREAD);
    ...
    if (isGraph)
    {
        ...
        if (!finished && isStart == true) {
            ...
            if (current_state.algo == hdist::Algorithm::Jacobi) {
                ...
                {
#ifdef debug
                    //printf("%d th thread: from %d to %d\n", i, displs[i], displs[i]+scounts[i]);
#endif
                    thread_pool.push_back(std::thread{hdist::calculate, std::ref(current_state), std::ref(grid), displs[i], scounts[i], k_turn});
                }
...
#ifdef debug
            if (current_state.algo == hdist::Algorithm::Jacobi) {
                hdist::calculate(current_state, grid1, 0, current_state.room_size, k1);
                grid1.switch_buffer();
            }
            else {
                hdist::calculate(current_state, grid1, 0, current_state.room_size, k1);
    ...
    else if (isGraph == false)
    {
        ...
        while (finished == false)
        {
            ...
            if (current_state.algo == hdist::Algorithm::Jacobi)
            {
                ...
                //     for (int j = 0; j < current_state.room_size; ++j)
                //     {
                //         printf("[%d %d] buffer: %f %f\n", i, j, grid[{i, j}], grid[{alt, i, j}]);
                //     }
                // }
#endif
            }
